...
两轮对比实体数 / event order / applied Tick。SUCCESS 仅当 sibling `lumio-mvp-host` 实际准入 101 路活连接。第 65 路 503、Admission 未入 FullGraph、origin/main Account Server / mvp-host 产物缺失必须记 BLOCKED（`blocked.json` + 文件:行 + 实测错误），不得回退 r-00344，不得用 `GameRoomHost` 伪造 SUCCESS。
