...
- **收口门槛:** `node .spec/tools/spec-lint.mjs && node --test .spec/tools/spec-lint.test.mjs && dotnet build LumioGame.sln && dotnet test LumioGame.sln`；`dotnet` 两条需要同级 `LumioGameRuntime` 检出(或 `LUMIO_RUNTIME_ROOT` 指向)。公共语义要改先去架构仓 `LumioGameEngine`，不在本仓自行改写；交付前必须通过。
