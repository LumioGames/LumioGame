...
export function buildDangerMap(board: Board, dangerTicks: number, virtual?: VirtualBomb): DangerMap {
  ...
  let bombAt = board.bombAt
  if (virtual) {
    const cell = idx(virtual.X, virtual.Y, board.size)
    bombs.push({ cell, X: virtual.X, Y: virtual.Y, power: virtual.power, fuseEndTick: virtual.fuseEndTick, id: 0, owner: 0 })
    bombAt = board.bombAt.slice()
    bombAt[cell] = bombs.length - 1
  }
  ...
  for (let pass = 0; pass < MAX_PASSES; pass++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      bl.bricks.length = 0
      const t = det[i]
      traceBlast(board, bombs[i].X, bombs[i].Y, bombs[i].power, bl, (c) => doomed[c] < t)
    }
    done.fill(0)
    for (let k = 0; k < n; k++) {
      let best = -1
      ...
      for (const c of blasts[best].covered) {
        const j = bombAt[c]
        if (j >= 0 && j !== best && det[j] > det[best]) det[j] = det[best]
      }
    }
  ...
  return { from, until, det, doomedAt: doomed, poison: board.poisonAt }
}
...
export function restsAt(dm: DangerMap, c: number, t: number): boolean {
  return dm.from[c] === NEVER || dm.until[c] + 2 <= t
}

/** 在 [enter, leave) 占用格 c 是否与其危险窗重叠；两端各留 margin（默认 2）Tick 吸收位置 / 取整误差。 */
export function conflicts(dm: DangerMap, c: number, enter: number, leave: number, margin = 2): boolean {
  return enter - margin < dm.until[c] && leave + margin > dm.from[c]
}
