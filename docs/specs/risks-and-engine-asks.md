...
| 产品 | 影响 | 判定 |
|---|---|---|
| ① 炸弹人（封顶 7 分钟的滚动局） | 掉线丢当前强化与拾取的技能（角色专属技能不丢），重新加入即可回场（滚动房间天然缓解，见 ADR-0006 / 0011）；决赛圈内掉线等于出局，由决赛圈 ≤ 115 秒与整局封顶 7 分钟框住（ADR 0025 / 0031） | 可接受 |
...
3 槽技能 + 4 个角色专属技能 + 组合进化把 ① 从「极简底盘」扩成带局内成长的产品，首发范围与验证周期都变大。

**应对**（ADR 0014 / 0030）：技能与角色放在 Stage 5 可选切片，首测 7 个技能 + 3 条组合；只有核心模式通过 Stage 4 的留局门与可读性门才开启，恶化则关 Feature Flag，技能关闭、角色只剩外观（设计稿 §8 / §16）；网页原型已提前实现，仅限原型。
