...
| 文档 | 状态 | 一句话 |
|---|---|---|
| [bomber/design.md](bomber/design.md) | 设计中 | 体素炸弹人完整设计稿:100 人 .io 帽王乱斗与滚动房间、开局四选一角色与专属技能、技能糖与组合进化、三心血量与连锁结算、决赛圈一路缩到 1×1 与「活到最后者胜」、死亡掉强化、人数-地图联动与双密度指标、场景材质、分区补给箱与中央定时补给、可读性功能、首轮默认值、Gate 0 + Stage 0–6 切片与遥测门槛——首发产品(阶梯 ①) |
| [bomber/stage0-kernel-contract.md](bomber/stage0-kernel-contract.md) | 已冻结 v2.0.0 | Stage 0a 内核契约:Runtime 接入核验结论(ADR 0015)、6 个 Component/EntityType、命令/事件/端口、Config Schema、Scenario 文件格式——做 G-1..G-7/C-1 实现卡前查 |
| [bomber/stage0-test-matrix.md](bomber/stage0-test-matrix.md) | 已冻结 v2.0.0 | Stage 0a 风险驱动用例矩阵,按传播/连锁/血量/帽子/拾取/地图/回放/遥测分组,每条标消费卡与自动化层级——G-1..G-7 写测试前查 |
