...
{
    partial void OnAccountIdChanging(string old, string @new, ChangeReason reason);
    ...
    void IGeneratedComponent.CapturePersist(IPersistWriter writer)
    {
        ...
        writer.WriteString("IdentityComponent.accountId", AccountId.Value);
    ...
    void IGeneratedComponent.CaptureSync(IPersistWriter writer)
    {
        ...
        {
            prediction.WritePredictionField("IdentityComponent.accountId", AccountId.Value);
    ...
    void IGeneratedComponent.RestorePersist(IPersistReader reader)
    {
        if (reader.TryReadString("IdentityComponent.accountId", out string accountIdRestore))
            AccountId.SetSilent(accountIdRestore);
    ...
    {
        if (string.Equals(fieldId, "accountId", StringComparison.Ordinal))
        {
            if (silent) AccountId.SetSilent((string)value!);
            else AccountId.Value = (string)value!;
            return;
        }
